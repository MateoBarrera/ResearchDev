...
class Criteria:
    ...
    def __criteria(self, df):
        ...
        for evaluation in range(len(df)):
            ...
            wc, ci = self.__criteria_matriz_by_expert(df.iloc[evaluation], name_matrix=name_matrix)
            list_wc.append(wc)
            list_ci.append(ci)
        self.weight_criteria = self.weight_criteria.assign( wc = list_wc, CI_wc = list_ci)
    ...
    def __subcriteria(self, df):
        ...
        wsubs_keys = ['Ambiental', 'Económico', 'Técnico', 'CI_Ambiental', 'CI_Económico', 'CI_Técnico']
        for evaluation in range(len(df)):
            wsubs = {'Ambiental':None,  'Económico':None, 'Técnico':None,'CI_Ambiental':None, 'CI_Económico':None,'CI_Técnico':None}
            
            df_subs = df.iloc[evaluation]
            i = 0
            for batch in range(0, len(df_subs),6):
              name_matrix = f"Subcriterios {wsubs_keys[i]} - experto {evaluation + 1}"
              wc, ci = self.__criteria_matriz_by_expert(df_subs.iloc[batch:batch+6], name_matrix=name_matrix)
              wsubs[wsubs_keys[i]] = wc
              wsubs[wsubs_keys[i+3]] = ci
              i+=1
    ...
    def show_info(self):
        ...
        series = {"Experto": self.weight_criteria.index, "[CI Criterios]":self.weight_criteria['CI_wc'].tolist()}
        df = pd.DataFrame(series)
        print_table("Consistencia", df)
