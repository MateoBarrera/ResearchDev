...
class Criteria:
    ...
    def graph_weighs(self, criteria):
        ...
        df = pd.DataFrame(
            data,
            columns=["Ambiental", "Económico", "Técnico"],
            index=["Fluidez", "% Savings", "Escalabilidade"],
        )
        df_dummy = df.copy()
        df_dummy = df_dummy * 100
        percentage_values = [
            str(round(item, 1)) for item in df_dummy.values.flatten().tolist()
        ]
    ...
    def __subcriteria(self, df_file):
        ...
        for column in df_matrix_subs:
            self.weight_criteria[column] = self.__aggregation(
                df_matrix_subs[column].to_list()
            )

        if self.fuzzy_treatment:
            for column in df_matrix_subs:
                self.fuzzy_weight_criteria[column] = self.__fuzzy_conversion(
                    df_matrix_subs[column].to_list()
                )
    ...
    def __fuzzy_aggregation(self, df_file):
        ...
        for row in range(rows):
            ...
            for column in range(columns):
                result = self.__fuzzy_mean_geometric(
                    [x for x in df_file[:, row, column]]
                )
    ...
    def __defuzzification(self, fuzzy_weights):
        ...
        for index, number_fuzzy in enumerate(fuzzy_weights):
            defuzzy_weights[index] = (
                number_fuzzy[0] + 4 * number_fuzzy[1] + number_fuzzy[2]
            ) / 6
        return defuzzy_weights
